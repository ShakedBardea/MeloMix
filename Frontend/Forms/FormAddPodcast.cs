...
namespace Project_Listening.Frontend.Forms
{
    ...
    {
        private BindingList<Backend.Model.Comedy> comedys=new BindingList<Backend.Model.Comedy>();
        ...
        private void comboType_SelectedIndexChanged_1(object sender, EventArgs e)
        {
            ...
            {
                BindingList<Backend.Model.Comedy> comedys = ListeningManager.GetSpecificListenings<Backend.Model.Comedy>();
                dataGridViewPodcast.DataSource = ListeningManager.GetSpecificListenings<Podcast>();
        ...
        private void buttonAddPodcast_Click(object sender, EventArgs e)
        {
            if (IsValidForm())
            {
                ...
                else if(comboType.SelectedItem.ToString() == "Comedy")
                {
                    ...
                    Backend.Model.Comedy comedy = new Backend.Model.Comedy(name, publicationDate, time, language, type, nameCreator, target, isComedian);
                    ListeningManager.AddListening(comedy);
                    dataGridViewPodcast.DataSource = ListeningManager.GetSpecificListenings<Backend.Model.Comedy>();
                    comedys.Add(comedy);
                } 
        ...
        /////// you have to select Type and then select which of the song to update/////////
        private void buttonUpdatePod_Click(object sender, EventArgs e)
        {
            ...
            string target = comboTargetGroup.Text;
            if (type == "" || type == "None")
                MessageBox.Show("you need to select Type and then choose from the data", "OK", MessageBoxButtons.OKCancel);
            else
            {
                if (type == "Educational")
                {
                ...
                if (type == "Educational")
                {
                    ...
                    string topic = topicTextBox.Text;
                    bool hasSpecialization = checkBoxIsCreatorSpecialization.Checked;
                    selectedRow.Cells["Topic"].Value = topic;
        ...
        private void dataGridViewPodcast_MouseClick(object sender, MouseEventArgs e)
        {
            if (dataGridViewPodcast.SelectedCells.Count > 0)
            {
                ...
                if (comboType.Text == " ")
                    MessageBox.Show("you need to select Type and the choose from the data", "OK", MessageBoxButtons.OKCancel);
                else
                {
       ...
       /////// you have to select Type and then select which of the song to delete///////////
            ...
            if (MessageBox.Show("are you sure???", "Delete", MessageBoxButtons.OKCancel, MessageBoxIcon.Warning) == DialogResult.OK)
            {
                if (dataGridViewPodcast.SelectedCells.Count > 0)
                {
                    ...
                    else
                    {
                        ...
                        {
                            BindingList<Backend.Model.Comedy> comedys = ListeningManager.GetSpecificListenings<Backend.Model.Comedy>();
                            foreach (Backend.Model.Comedy comedy in comedys)
                            {
